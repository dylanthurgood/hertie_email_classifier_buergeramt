...
# Simple German stopwords (extend as needed)
GERMAN_STOPWORDS = {
    "und","der","die","das","in","den","von","mit","ist","ich","nicht","zu","für","auf",
    "ein","eine","im","dem","an","als","auch","am","sind","hat","war","oder","wir",
    "sie","er","es","dass","mehr","bei"
...
def tokenize(text):
    if not isinstance(text, str):
        return []
    # extract words (letters, digits allowed, >=2 chars)
    tokens = re.findall(r"(?u)\b\w{2,}\b", text.lower())
    # remove stopwords
    return [t for t in tokens if t not in STOP_WORDS]

def exclusive_words_by_category(df, text_col="Nachrichtentext", label_col="Klassen-Label"):
    random.seed(seed)
    # collect unique words per class
    class_vocab = {
        label: set(sum((tokenize(t) for t in group[text_col].dropna().astype(str)), []))
        for label, group in df.groupby(label_col)
    }
    # compute exclusives: words that appear only in that class
    exclusives = {}
    ...
    # Print random samples per class
...
# ---------------- prediction helper ----------------
def predict_email(df, index, model, class_names, text_column):
    """
    Predict the class and probabilities for a given email in a DataFrame.

    Parameters
    ----------
    df : pandas.DataFrame
        DataFrame containing the email texts.
    index : int
        Row index of the email to classify.
    model : sklearn pipeline or classifier
        Trained model with `predict` and `predict_proba` methods.
    class_names : list
        List of class names in the same order as model.classes_.
    text_column : str, optional
        Name of the column containing the text (default: 'Nachrichtentext').

    Returns
    -------
    str
        Predicted label for the given email.
    """
    # Extract email text by index
    email_text = df.loc[index, text_column]

    # Predict class and probabilities
    pred_class = model.predict([email_text])[0]
    ...
    # Combine and sort probabilities
    prob_pairs = sorted(zip(class_names, probs), key=lambda x: x[1], reverse=True)
    for label, p in prob_pairs:
        print(f"{label}: {p:.3f}")

    print(f"\n✅ Vorhergesagte Klasse: {pred_label}")

    return pred_label

def predict_own_email(df, model, class_names, email_text):
    """
    Predict the class and probabilities for a given email in a DataFrame.
...
def show_misclassifications(model, X_test, y_test, class_names, n_examples=5):
    """
    Show misclassified examples from model predictions on test data.
    ...
    X_test : pandas.Series or array-like
        Test features (e.g., text data).
    y_test : array-like
        True labels for test data.
    class_names : list
        List of class names (in same order as model.classes_).
    n_examples : int, optional
        Number of misclassified examples to print (default=5).
    ...
    print(f"🔍 Anzahl der falsch klassifizierten Beispiele: {len(wrong_idx)}")

    # Create DataFrame for comparison
    errors_df = pd.DataFrame({
        "Text": X_test if isinstance(X_test, pd.Series) else pd.Series(X_test),
        "True_Label": [class_names[int(i)] for i in y_test],
        "Predicted_Label": [class_names[int(i)] for i in y_pred]
    })

    # Filter misclassified
    misclassified_df = errors_df.iloc[wrong_idx]

    # Show a few examples
    print("\nBeispiele für Fehlklassifikationen:\n")
    for i, row in misclassified_df.head(n_examples).iterrows():
        print(f"Index {i}:")
        print(f"Text: {row['Text']}")
        print(f"Wahr: {row['True_Label']} | Vorhergesagt: {row['Predicted_Label']}")
        print("-" * 60)

    return misclassified_df


# ---------- Helper: simple text cleaning ----------
def simple_clean(text: str) -> str:
    """
    Very simple text cleaner: lowercases, removes URLs, emails, non-alnum chars,
    collapses whitespace. Returns empty string for non-str input.
    """
    if not isinstance(text, str):
        return ""
    text = text.lower()
...
def load_data(csv_path: str,
    ...
    """
    Load CSV into a DataFrame, ensure text & label columns exist, clean text.
    rename_map: optional dict to rename columns, e.g. {'Nachrichtentext': 'text'}.
    """
    df = pd.read_csv(csv_path)
    if rename_map:
        df = df.rename(columns=rename_map)
    if text_col not in df.columns or label_col not in df.columns:
        raise ValueError(f"Benötigte Spalten nicht gefunden. Vorhanden: {df.columns.tolist()}")
    df[text_col] = df[text_col].fillna("").map(simple_clean)
    df = df[[text_col, label_col]].dropna()
    return df


# ---------- Build pipeline ----------
def build_pipeline(max_features: int = 30000,
                   ngram_range: tuple = (1, 2),
                   xgb_params: Optional[dict] = None) -> Pipeline:
    """
    Build a sklearn Pipeline with TfidfVectorizer + XGBClassifier.
    """
    tfidf = TfidfVectorizer(max_features=max_features, ngram_range=ngram_range)
    xgb = XGBClassifier(use_label_encoder=False, eval_metric="logloss", **(xgb_params or {}))
    pipeline = Pipeline([
        ("tfidf", tfidf),
        ("xgb", xgb)
    ])
...
# ---------- Train / evaluate (renamed to xgboost_classifier) ----------
def xgboost_classifier(df: Union[pd.DataFrame, str],
                       text_col: str = "text",
                       label_col: str = "label",
                       test_size: float = 0.2,
                       random_state: int = 42,
                       ...
                       xgb_default_params: Optional[dict] = None,
                       verbose: bool = True) -> Tuple[Pipeline, LabelEncoder, dict]:
    """
    Train and evaluate an XGBoost classifier on text data.
    - df: either a pd.DataFrame or a path to a CSV file
    - returns: (trained_pipeline, label_encoder, metrics_dict)
    metrics_dict contains accuracy, classification_report (as dict), confusion_matrix (np.array)
    """
    # Accept csv path or DataFrame
    if isinstance(df, str):
        df = pd.read_csv(df)

    # auto-rename common German columns (convenience)
    if "Klassen-Label" in df.columns:
        df = df.rename(columns={"Klassen-Label": "label"})
    if "Nachrichtentext" in df.columns:
        df = df.rename(columns={"Nachrichtentext": "text"})
    ...
    # default xgb params
    xgb_params = xgb_default_params or {
        "n_estimators": 200,
        "max_depth": 6,
        "learning_rate": 0.1,
        "n_jobs": -1,
    ...
    pipeline = build_pipeline(max_features=25000, ngram_range=(1, 2), xgb_params=xgb_params)
    ...
    clf_report = classification_report(y_test, y_pred, target_names=list(le.classes_), output_dict=True)
...
# ---------- Inference helper ----------
def predict_texts(model_pipeline: Pipeline, label_encoder: LabelEncoder, texts: List[str]) -> List[str]:
    """
    Predict labels for a list of raw texts (cleaning applied).
    Returns decoded label names.
    """
    cleaned = [simple_clean(t) for t in texts]
    preds = model_pipeline.predict(cleaned)
    return label_encoder.inverse_transform(preds).tolist()


# ---------- Save / Load ----------
def save_model(model_pipeline: Pipeline,
               label_encoder: LabelEncoder,
               out_dir: str = "models",
               name: str = "xgb_email_clf") -> Tuple[Path, Path]:
    """
    Save pipeline and label encoder with joblib. Returns paths.
    """
    Path(out_dir).mkdir(parents=True, exist_ok=True)
    model_path = Path(out_dir) / f"{name}.joblib"
    le_path = Path(out_dir) / f"{name}_label_encoder.joblib"
...
def load_model(model_path: str, le_path: str) -> Tuple[Pipeline, LabelEncoder]:
    """
    Load model pipeline and label encoder saved with save_model.
    """
