...
def clean(df, column_name):
    """
    Remove English and German stopwords from a pandas DataFrame text column.
    ...
    -------
    pandas.Series
        A cleaned text column with stopwords removed.
    """
    cleaned = (
        df[column_name]
        .astype(str)  # make sure it's string
        .apply(lambda x: re.findall(r'\b\w+\b', x.lower()))  # tokenize by words
        .apply(lambda tokens: " ".join([w for w in tokens if w not in STOP_WORDS]))  # remove stopwords
    )
    return cleaned
